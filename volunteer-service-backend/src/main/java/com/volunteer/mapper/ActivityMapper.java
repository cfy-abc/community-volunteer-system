...
public interface ActivityMapper {
    ...
    @Insert("INSERT INTO activity(title, description, start_time, end_time, location, longitude, latitude, max_participants, current_participants, reward_hours, creator_id, org_id, status, create_time, poster, contact_phone, type, tags, conditions, feedbacks, sign_method) " +
            "VALUES(#{title}, #{description}, #{startTime}, #{endTime}, #{location}, #{longitude}, #{latitude}, #{maxParticipants}, 0, #{rewardHours}, #{creatorId}, #{orgId}, #{status}, NOW(), #{poster}, #{contactPhone}, #{type}, #{tags}, #{conditions}, #{feedbacks}, #{signMethod})")
    ...
    @Select("<script>" +
            "SELECT * FROM activity AS a WHERE 1=1 " +
    ...
    @Update("UPDATE activity SET status = 1 WHERE status = 4")
    int fixPendingActivities();
}
